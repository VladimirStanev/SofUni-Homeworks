function attachEvents() {
    ...
    otherDOMSelectors.loadBtn.addEventListener('click', loadAllProductsTask);
    otherDOMSelectors.updateBtn.addEventListener('click', updateProductsTask);
    otherDOMSelectors.addBtn.addEventListener('click', addProductTask);

    function loadAllProductsTask(event) {
        if(event) {
        ...
        fetch(BASE_URL)
            ...
            .then((allProductsRes) => {
                ...
                for(const { product, count, price, _id } of currentProducts) {
                    ...
                    updateBtn.addEventListener('click', loadUpdateFormTask);
                    deleteBtn.addEventListener('click', deleteProductTask);
                }
            })

            console.log(currentProducts.JSON());
    }

    function loadUpdateFormTask() {
    ...
    function updateProductsTask(event) {
        event.preventDefault();
        const { product, count, price } = inputDOMSelectors;
        const payload = JSON.stringify({
            product: product.value,
